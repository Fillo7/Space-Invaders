...
HRESULT InitD3D(HWND hWnd, const bool fullscreen)
{
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR cmd, INT)
{
    ...
    hWnd = CreateWindow("Game", "Polished Crap", style, 0, 0, r.right - r.left, r.bottom - r.top, GetDesktopWindow(), NULL, wc.hInstance, NULL);
...
void DrawSprite(void* sprite, float xCentre, float yCentre, float xSize, float ySize, float rotationAngleRadians, DWORD tintColorARGB)
{
    ...
    {
        // x, y, z, rhw, color
        {ROTATE(-xSize, -ySize), 0.5f, 1.0f, tintColorARGB, 0, 0},
