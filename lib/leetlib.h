...
void DrawSprite(void* sprite, float xCentre, float yCentre, float xSize, float ySize, float rotateAngleRadians = 0, DWORD tintColorARGB = 0xffffffff);

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// sound
int PlayMusic(const std::string& filename, float volume = 1); // returns a handle which you only need if you want to change volume later with ChangeVolume
void StopMusic();

// sorry for this being 'snd' but otherwise it clashes with a windows #define (PlaySound) grr
void* LoadSnd(const std::string& filename, bool loop = false); // if you set loop the sample will go on looping forever, until you call StopSound
int PlaySnd(void* sound, float volume = 1.0f); // returns a handle which you only need if you are going to call StopSound or ChangeVolume()
